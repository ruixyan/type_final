...
function handleDwellClick(normalizedX, normalizedY) {
    ...
    if (isClickable) {
        ...
        if (dwellElement === targetElement) {
            ...
            const progress = Math.min(dwellTime / 3000, 1); // 3 seconds
            
            // Update progress indicator
            updateDwellProgress(x, y, progress);
            
            if (dwellTime >= 3000 && !dwellTimeout) {
...
function updateUI() {
    ...
    if (gestureState.isHandDetected) {
        ...
        if (activeSlider) {
            statusText.textContent = 'Adjusting slider';
        } else if (gestureState.isPinching) {
            statusText.textContent = 'Pinch & drag';
