...
function createGrid(container, chars) {
    ...
    chars.forEach(char => {
        ...
        cell.onmouseenter = () => previewGlyph(char);
        cell.onmouseleave = () => restoreGlyph();
        cell.onclick = () => selectGlyph(char);
...
document.querySelectorAll('.font-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        document.querySelectorAll('.glyph-cell').forEach(cell => {
            cell.className = `glyph-cell ${currentFont}`;
            if (cell.textContent === currentChar) cell.classList.add('active');
        });
...
// Make functions and variables global for gesture control
window.previewGlyph = previewGlyph;
window.restoreGlyph = restoreGlyph;
window.updateGlyphDisplay = updateGlyphDisplay;
window.currentChar = currentChar;
window.currentFont = currentFont;

// Watch for changes to currentFont and currentChar from gesture control
Object.defineProperty(window, 'currentFont', {
    get: () => currentFont,
    set: (value) => {
        currentFont = value;
    }
...
Object.defineProperty(window, 'currentChar', {
    get: () => currentChar,
    set: (value) => {
        currentChar = value;
    }
